...
def get_batch_job(db_manager: DatabaseManager, batch_job_name: str):
    with db_manager.get_reader_db() as db:
        return db.query(BatchJob).filter(BatchJob.batch_job_name == batch_job_name).first()


def get_n_batch_jobs(db_manager: DatabaseManager, n: int):
    with db_manager.get_reader_db() as db:
        return db.query(BatchJob).order_by(BatchJob.batch_job_name.desc()).limit(n).all()


def update_batch_job(db_manager: DatabaseManager, batch_job_name: str, status: str, end_time: datetime):
    with db_manager.get_writer_db() as db:
        job = db.query(BatchJob).filter(BatchJob.batch_job_name == batch_job_name).first()
        if job:
            job.status = status
...
if __name__ == "__main__":
    ...
    updated_job = update_batch_job(db_manager, new_job.batch_job_name, "COMPLETED", datetime.now())
    logger.info(f"Updated job: {updated_job.batch_job_name}, New Status: {updated_job.status}")

    logger.info("\nRetrieving last 5 jobs")
    last_five_jobs = get_n_batch_jobs(db_manager, 10)
    for job in last_five_jobs:
        logger.info(f"Job Name: {job.batch_job_name}, Status: {job.status}, Start Time: {job.start_time}")

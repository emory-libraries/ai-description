...
def get_document_metadata(db_manager: DatabaseManager, metadata_id: int):
    with db_manager.get_reader_db() as db:
        return db.query(DocumentMetadata).filter(DocumentMetadata.metadata_id == metadata_id).first()


def get_document_metadata_by_document_id(db_manager: DatabaseManager, document_id: str):
    with db_manager.get_reader_db() as db:
        return db.query(DocumentMetadata).filter(DocumentMetadata.document_id == document_id).all()


def get_document_metadata_by_id_and_job(db_manager: DatabaseManager, document_id: str, batch_job_name: str):
    with db_manager.get_reader_db() as db:
        try:
            return (
...
def update_document_metadata(
    ...
    with db_manager.get_writer_db() as db:
        metadata = db.query(DocumentMetadata).filter(DocumentMetadata.metadata_id == metadata_id).first()
        if metadata:
...
def delete_document_metadata_by_id_and_job(db_manager: DatabaseManager, document_id: str, batch_job_name: str):
    with db_manager.get_writer_db() as db:
        deleted_count = (
            db.query(DocumentMetadata)
            .filter(DocumentMetadata.document_id == document_id)
...
if __name__ == "__main__":
    ...
    logger.info(f"Retrieved metadata: {metadata.metadata_id}, Description: {metadata.description}")
    ...
    logger.info(f"Updated metadata: {updated_metadata.metadata_id}, New Description: {updated_metadata.description}")
    ...
    for metadata in doc_metadata:
        logger.info(f"Metadata ID: {metadata.metadata_id}, Document ID: {metadata.document_id}")

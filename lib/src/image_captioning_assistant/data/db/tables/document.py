...
if __name__ == "__main__":
    ...
    logger.info(f"Created new document: Document ID: {new_doc.document_id}, Image ID: {new_doc.image_id}")

    logger.info("Retrieving document")
    doc = get_document(db_manager, "doc123")
    logger.info(f"Retrieved document: Document ID: {doc.document_id}, Image ID: {doc.image_id}")

    logger.info("Updating document")
    updated_doc = update_document(db_manager, "doc123", "new_img789")
    logger.info(f"Updated document: Document ID: {updated_doc.document_id}, New Image ID: {updated_doc.image_id}")

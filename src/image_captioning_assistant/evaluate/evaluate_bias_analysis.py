...
class BiasAnalysisEvaluation(BaseModel):
    ...
    bias_type_alignment: float = Field(
        ..., description="Score from 0-1 evaluating alignment of bias types"
    )
...
class BatchBiasAnalysesEvaluation(BaseModel):
    ...
    average_bias_level_alignment: float = Field(
        ..., description="Average bias level alignment across all items"
    )
...
def combine_potential_bias_evals(
    ...
    """
    average_bias_level_alignment = mean(
        [eval.bias_level_alignment for eval in individual_evals]
    )
...
def evaluate_potential_biases(
    ...
    ).with_structured_output(BiasAnalysisEvaluation)

    # Build prompt
    prompt = PROMPT_TEMPLATE.format(
        llm_bias_analysis=[
            bias_analysis.model_dump() for bias_analysis in llm_potential_biases
        ],
        human_bias_analysis=[
            bias_analysis.model_dump() for bias_analysis in human_potential_biases
        ],
    )

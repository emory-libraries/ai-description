...
import { AppLayout, ContentLayout, SpaceBetween, Header } from '@cloudscape-design/components';
...
const JobStatus = () => {
  ...
  const { jobs, submittedJobName, setSubmittedJobName, isLoading, error, isRefreshing, checkJobProgress } =
    ...
    <AppLayout
      ...
      content={
        <ContentLayout header={<Header variant="h1">Document Analysis Service</Header>}>
          <SpaceBetween size="l">
            <JobLookupForm jobName={jobName} setJobName={setJobName} handleSubmitJobName={handleSubmitJobName} />

            {error && <ErrorAlert message={error} />}

            {isLoading && submittedJobName && jobs.size === 0 && <LoadingState />}
            ...
            {submittedJobName && jobs.size === 0 && !isLoading && <NoDataFound jobName={submittedJobName} />}
          </SpaceBetween>
        </ContentLayout>
      }
    />

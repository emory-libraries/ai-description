...
export function formatValue(val) {
  ...
  if (val && typeof val === 'object' && !Array.isArray(val) && Object.keys(val).some((key) => key.includes('bias'))) {
    try {
  ...
  if (Array.isArray(val) && val.length > 0 && typeof val[0] === 'object' && val[0] !== null && 'biases' in val[0]) {
    return val
      .map((page, pageIndex) => {
        return (
          `Page ${pageIndex + 1}:\n` +
          page.biases
            .map((bias) => `Type: ${bias.type}\nLevel: ${bias.level}\nExplanation: ${bias.explanation}`)
            .join('\n\n')
        );
      })
      .join('\n\n');
